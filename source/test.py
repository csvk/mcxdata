...
def compare_test_2(conn, hist, symbol, leeway):
    ...
    ra_hist = read_history(hist, symbol + '-I')  # Ratio-adjusted history
    #print(symbol, ra_hist)
...
def compare_test_3(conn, hist, symbol, leeway):
    """
    Compare test results for trade entry on last day of expiry in ratio adjusted continuous data and exit on firstt day
    of expiry: between ratio adjusted continuous data vs unadjusted data
    :return: None
    """
...
for key, data in exp['expiry_dates'].items():
    messages.append(compare_test_3(conn, hist, str.strip(key), 0.0001))
